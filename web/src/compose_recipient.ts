...
export function initialize(): void {
    compose_select_recipient_dropdown_widget = new dropdown_widget.DropdownWidget({
        ...
        tab_moves_focus_to_target(e: JQuery.KeyDownEvent): string | undefined {
            if (e.shiftKey) {
                const $last_banner = $("#compose_banners .cf-notice").last();
                ...
                if ($last_banner.find(".cf-notice__close").length > 0) {
                    return "#compose_banners .cf-notice__close:last";
                }
                if ($last_banner.find(".cf-notice__action").length > 0) {
                    return "#compose_banners .cf-notice__action:last";
                }
                return undefined;
            }
