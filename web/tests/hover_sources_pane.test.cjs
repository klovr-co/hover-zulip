...
const focused_result = {focus_count: 0, focus() { this.focus_count += 1; }};
const pane = {
    hidden: true,
    innerHTML: "",
    querySelector() {
...
run_test("owns loading, stale-response, loaded, error, and focus state", ({override, mock_template}) => {
    const renders = [];
    const requests = [];
    ...
    const first_trigger = {isConnected: true, focus_count: 0, focus() { this.focus_count += 1; }};
    const second_trigger = {isConnected: true, focus_count: 0, focus() { this.focus_count += 1; }};
